async function createProduct() {
    ...
    try {
        ...
        alert('Might take few seconds to process your request. You will be notified once the product is added to bloackchain.')
...
async function addHistory() {
    try {
        ...
        console.error(error);
        alert('An error occurred while adding history. Please try again.');
    }
}
...
async function getProductCount() {
    try {
        ...
        console.error(error);
        alert('An error occurred while fetching product count. Please try again.');
    }
}
...
async function getProduct() {
    try {
        ...
        await contract.methods.getProduct(productId).call({ from: accounts[0] }).then(async result => {
            console.log(result.sealHash);
